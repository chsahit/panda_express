...
def rtb_IK(X_WG: np.ndarray, q0: np.ndarray, clean: bool = True):
    if clean:
...
def goto_hand_position(rob: BambooFrankaClient, X_WG: np.ndarray, time: float, n_ik_attempts: int = 5) -> int:
