...
from panda_express.skills.go_to_conf import goto_hand_position, TOP_DOWN_GRASP_ROT, rtb_IK, goto_joint_angles
...
def _get_pixel_from_gemini(vlm_query_str: str, pil_image: Image.Image) -> Tuple[int, int]:
    ...
    if not isinstance(parsed_data, list) or not parsed_data:
        raise ValueError(f"Parsed JSON {parsed_data} is not a non-empty list.")
...
Choose a placement point that is in the middle of the object's surface.
...
Coordinates MUST be normalized to 0-1000."""
    ...
    save_folderpath = "image_logs/placing/"
    ...
    z_init = np.array([0, 0.0, robot.get_joint_states()["ee_pose"][2][3]])
    ...
    X_WPregrasp[:3, 3] = pixel_xyz + z_init + np.array([0.0, 0.0, 0.1])
    goto_hand_position(robot, X_WPregrasp, 5.0)

    X_WGoal = np.eye(4)
    X_WGoal[:3, :3] = TOP_DOWN_GRASP_ROT
    X_WGoal[:3, 3] = pixel_xyz + z_init + np.array([0.0, 0.0, 0.03])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text_prompt", type=str, default="tray", help="Text prompt for the surface to place on")
    args = parser.parse_args()
    with BambooFrankaClient(server_ip="128.30.224.88") as rob:
        place_at(rob, args.text_prompt)
